...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def backward(ctx, ans_grad: Tensor):
        ...
        if ctx.activation == "SwooshL":
            y = SwooshLForward(x)
            func_deriv = torch.sigmoid(x - 4.0) - 0.08
        elif ctx.activation == "SwooshR":
            y = SwooshRForward(x)
            func_deriv = torch.sigmoid(x - 1.0) - 0.08
        else:
            raise KeyError(f"Unsupported activation: {ctx.activation}")
        if dropout_mask is not None:
            y = y * dropout_mask
