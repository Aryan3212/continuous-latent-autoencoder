...
#
# Idempotent: uv sync is incremental and the HF snapshot fetch verifies files
# it already has. All credentials are read from the environment (sourced from
# .env below) — no generated creds file.
#
# Only HF_TOKEN is required here: training fetches the *packed* dataset repo
# (HF_DATASET_REPO) built once by `make pack-and-push`. Kaggle keys are needed
...
if [ -z "${HF_TOKEN:-}" ]; then
    ...
    echo "        dataset repo ($HF_DATASET_REPO). Put it in .env (see .env.example)"
...
export DATA_ROOT HF_DATASET_REPO HF_MODEL_REPO
...
# --- 4. Data: fetch the packed dataset snapshot ------------------------------
make fetch-data DATA_ROOT="$DATA_ROOT" HF_DATASET_REPO="$HF_DATASET_REPO"

for m in train.jsonl val.jsonl; do
    if [ ! -s "$DATA_ROOT/manifests/$m" ]; then
        echo "[setup] ERROR: $DATA_ROOT/manifests/$m missing or empty after fetch."
        exit 1
    fi
done
echo "[setup] manifests: $(wc -l < "$DATA_ROOT/manifests/train.jsonl") train rows," \
     "$(wc -l < "$DATA_ROOT/manifests/val.jsonl") val rows"

# --- 5. Train ----------------------------------------------------------------
if [ "$NO_TRAIN" = "1" ]; then
    echo "[setup] --no-train: done. Start later with:"
    echo "        make train DATA_ROOT=$DATA_ROOT CONFIG=$CONFIG"
    exit 0
fi
