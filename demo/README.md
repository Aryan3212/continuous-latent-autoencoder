...
waveform. Runs on CPU (free Spaces tier is fine).
...
4. The Space builds, clones the model code from GitHub at startup, downloads `last.pt`,
   and serves the UI. First boot takes ~1–2 min (clone + ckpt download + model load).
...
- To pin a specific checkpoint, change `HF_FILE` / `HF_REPO` in `app.py`; to change
  the model config, change `CONFIG_PATH`.
